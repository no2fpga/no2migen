...
# with a CPU, its ROM (in SPI Flash), its SRAM, and a USB CDC ACM UART,
...
class BaseSoC(SoCCore):
    ...
    def __init__(self, bios_flash_offset, revision="v1", **kwargs):
        ...
        self.submodules.uart = NitroMuAcmUart(platform, usb_pads, product="bitsy LiteX μACM")
        self.add_constant("UART_POLLING")

        # DFU RT reboot support --------------------------------------------------------------------
        reboot = Signal()
        self.sync += reboot.eq(reboot | self.uart.bootloader_req)
